    ...
    ui(new Ui::menu_dialog)
{
    ...
    ui->buscar_LE->setPlaceholderText(QString::fromLatin1("Buscar título o autor"));

    ui->buscar_misLibros_LE->setPlaceholderText(QString::fromLatin1("Buscar título o autor"));
    ui->buscar_favoritos_LE->setPlaceholderText(QString::fromLatin1("Buscar título o autor"));

    QStringList opciones,opciones1;

    opciones<<"     "<<QString::fromLatin1("Título")<<"Autor"<<"Editorial"<<QString::fromLatin1("Año")<<"Tipo";
    ui->comboBox_sortLibros->addItems(opciones);
    opciones1<<"     "<<QString::fromLatin1("Título");
...
void menu_dialog::cargarLibros()
{
    ...
    query->prepare("select*from libro");
    query->exec();
    model->setQuery(*query);
    QString anio= QString::fromLatin1("Año");
    QString titulo= QString::fromLatin1("Título");
    model->setHeaderData(0,Qt::Horizontal, tr("Autor"));
    model->setHeaderData(1,Qt::Horizontal, tr("Editorial"));
    model->setHeaderData(2,Qt::Horizontal, titulo);
    model->setHeaderData(3,Qt::Horizontal, tr("Tipo"));
    model->setHeaderData(4,Qt::Horizontal, anio);
    ...
    query->prepare("select titulo from libro_usuario where usuario='"+ui->usuario_LE->text()+"'");
    query->exec();
    model2->setQuery(*query);


    model2->setHeaderData(0,Qt::Horizontal, titulo);

    ui->misLibros->setModel(model2);

    ui->misLibros->setEditTriggers(QAbstractItemView::NoEditTriggers);

    ui->misLibros->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
    ui->misLibros->verticalHeader()->setVisible(false);


    query->prepare("select titulo from libro_favorito where usuario='"+ui->usuario_LE->text()+"'");
    query->exec();
    model3->setQuery(*query);


    model3->setHeaderData(0,Qt::Horizontal, titulo);

    ui->favoritos->setModel(model3);

    ui->favoritos->setEditTriggers(QAbstractItemView::NoEditTriggers);

    ui->favoritos->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
...
void menu_dialog::on_buscar_LE_textChanged(const QString &arg1)
{
    ...
    if(ui->buscar_LE->text().size()>0)
    {
        ...
        QSqlQuery *query =  new QSqlQuery(data);
        query->prepare("select*from libro where titulo='"+buscar+"' or autor='"+buscar+"'");
        //"select titulo from libro_favorito where usuario='"+ui->usuario_LE->text()+"'"
        query->exec();
        model->setQuery(*query);
        ui->tableView->setModel(model);
...
void menu_dialog::grafo_recomendacion()
{
    ...
    //QHash <QStringTitulo, int> aristas;
...
void menu_dialog::on_comboBox_sortLibros_currentTextChanged(const QString &arg1)
{
    ...
    QSqlQuery *query1 =  new QSqlQuery(data);

    if(ui->comboBox_sortLibros->currentText()==QString::fromLatin1("Título"))
    {


        query1->prepare("select*from libro order by titulo");
        query1->exec();
    ...
    if(ui->comboBox_sortLibros->currentText()=="Autor")
    {
        ...
        query1->prepare("select*from libro order by autor");
        query1->exec();
        model->setQuery(*query1);
        ui->tableView->setModel(model);
    ...
    {
        query1->prepare("select*from libro order by editorial");
        query1->exec();
        model->setQuery(*query1);
        ui->tableView->setModel(model);
    ...
    if(ui->comboBox_sortLibros->currentText()==QString::fromLatin1("Año"))
    {
        query1->prepare("select*from libro order by anio");
        query1->exec();
        model->setQuery(*query1);
        ui->tableView->setModel(model);
    }
    if(ui->comboBox_sortLibros->currentText()=="Tipo")
    {
        ...
        query1->prepare("select*from libro order by tipo");
        query1->exec();
        model->setQuery(*query1);
        ui->tableView->setModel(model);
...
void menu_dialog::on_comboBox_sortMislibros_currentTextChanged(const QString &arg1)
{
    ...
    QSqlQuery *query2 =  new QSqlQuery(data);

    if(ui->comboBox_sortMislibros->currentText()==QString::fromLatin1("Título"))
    {
        query2->prepare("select titulo from libro_usuario where usuario='"+ui->usuario_LE->text()+"' order by titulo");
        query2->exec();
...
{
    QSqlQuery *query3 =  new QSqlQuery(data);
    if(ui->comboBox_sortFavoritos->currentText()==QString::fromLatin1("Título"))

    {
        query3->prepare("select titulo from libro_favorito where usuario='"+ui->usuario_LE->text()+"' order by titulo");
        query3->exec();
